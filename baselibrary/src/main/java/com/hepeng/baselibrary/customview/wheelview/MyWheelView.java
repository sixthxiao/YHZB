...
public class MyWheelView extends View {
    ...
    private void initPaints() {
        ...
        paintIndicator.setColor(getResources().getColor(R.color.white));
    ...
    public void smoothScroll(MyWheelView.ACTION action) {//平滑滚动的实现
        cancelFuture();
        if (action == MyWheelView.ACTION.FLING || action == MyWheelView.ACTION.DAGGLE) {
...
//                canvas.scale(1.0F, (float) Math.sin(radian));
            ...
            default:
                if (!eventConsumed) {//未消费掉事件
                    ...
                    if ((System.currentTimeMillis() - startTime) > 120) {
                        // 处理拖拽事件
                        smoothScroll(MyWheelView.ACTION.DAGGLE);
                    } else {
                        // 处理条目点击事件
                        smoothScroll(MyWheelView.ACTION.CLICK);
                    }
                }
                break;
        }
    ...
    public void setItemsVisible(int counts) {
        itemsVisible = counts;
    }
